...
```json
{
    "info": {
        ...
        "title": "Event Index Server", // [示例] 服务器名称
    ...
    "protocol": {
        "version": {
            "name": "2.0.0",                // 版本名称，用于展示给用户，不参与版本号判断
            "code": 20000,                  // 主版本标识，参与协议的版本号判断
    ...
    "security": {                           // 安全相关选项（可选，如无该字段标识不使用安全及加密选项）
        "encryption": {                     // 加密相关选项（可选）
            ...
            "keys": {                       // 密钥字段【本协议不做详细规定；请根据使用场景自行决定】
                ...
                "extension": {}             // 扩展字段（根据使用场景自行增加，本协议不做规定）           // 扩展字段（根据使用场景自行增加，本协议不做规定）
            },
